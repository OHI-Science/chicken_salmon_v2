...
```{r}
cellStats(broiler_cpi, "max", na.rm = T)
```

```{r}
cellStats(salmon_cpi, "max", na.rm = T)
```
...
```
Test CPI/km2 vs area correlation?

```{r}
cor.test(broiler_cpi_km2_prod$cum_stress_km2, broiler_cpi_km2_prod$total_area, type = "pearsons")
```

```{r}
cor.test(salmon_cpi_km2_prod$cum_stress_km2, salmon_cpi_km2_prod$total_area, type = "pearsons")
```

```{r}
broiler_cpi_comp<-full_join(iso3_broiler_cpi, broiler_cpi_km2)
```
...
cells_iso3_cpi<- iso3_cpi %>% 
  ...
  summarise(mean_broiler_cpi = mean(broiler_cpi, na.rm = T),
            mean_salmon_cpi = mean(salmon_cpi, na.rm = T),
            med_broiler_cpi = median(broiler_cpi, na.rm = T),
            med_salmon_cpi = median(salmon_cpi, na.rm = T),
    broiler_cells = length(which(broiler_cpi>0)),
         salmon_cells = length(which(salmon_cpi>0)),
         med_broiler = length(which(broiler_cpi>chick_quant_45 & broiler_cpi<chick_quant_90)),
         med_salmon = length(which(salmon_cpi>salmon_quant_45 & salmon_cpi<salmon_quant_90)),
         high_broiler = length(which(broiler_cpi>chick_quant_90)),
         high_salmon = length(which(salmon_cpi>salmon_quant_90))) %>% 
  mutate(prop_broiler = broiler_cells/ncells,
         prop_salmon = salmon_cells/ncells,
         prop_high_broiler = high_broiler/broiler_cells,
         prop_high_salmon = high_salmon/salmon_cells) %>% 
  filter(!is.na(iso3c) == T)

cells_iso3_cpi<-cells_iso3_cpi %>% 
  mutate(mean_broiler_global = mean(broiler_cpi_df$broiler_cpi, na.rm = T),
         mean_salmon_global = mean(salmon_cpi_df$salmon_cpi, na.rm = T),
         med_broiler_global = median(broiler_cpi_df$broiler_cpi, na.rm = T),
         med_salmon_global = median(salmon_cpi_df$salmon_cpi, na.rm = T))
