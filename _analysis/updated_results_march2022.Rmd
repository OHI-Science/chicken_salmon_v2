...
all_chick_dist<-sum(chicken_disturb_df$chickens_disturbance_per_cell)
...
chicken_fw<-raster("/home/shares/food-systems/Food_footprint/chicken_salmon/_tif_equal_area_proj_per_cell/gall_peter_farm_land_chickens_industrial_meat_water_per_cell.tif")

salmon_fw<-raster("/home/shares/food-systems/Food_footprint/chicken_salmon/_tif_equal_area_proj_per_cell/gall_peter_farm_marine_salmon_aquaculture_meat_water_per_cell.tif")

chicken_nutri<-raster("/home/shares/food-systems/Food_footprint/chicken_salmon/_tif_equal_area_proj_per_cell/gall_peter_farm_land_chickens_industrial_meat_nutrient_per_cell.tif")

salmon_nutri<-raster("/home/shares/food-systems/Food_footprint/chicken_salmon/_tif_equal_area_proj_per_cell/gall_peter_farm_marine_salmon_aquaculture_meat_nutrient_per_cell.tif")

chicken_ghg<-raster("/home/shares/food-systems/Food_footprint/chicken_salmon/_tif_equal_area_proj_per_cell/gall_peter_farm_land_chickens_industrial_meat_ghg_per_cell.tif")

salmon_ghg<-raster("/home/shares/food-systems/Food_footprint/chicken_salmon/_tif_equal_area_proj_per_cell/gall_peter_farm_marine_salmon_aquaculture_meat_ghg_per_cell.tif")
```
...
```{r}
all_c_fw<-sum(chicken_fw_df$gall_peter_farm_land_chickens_industrial_meat_water_per_cell,na.rm = T)
all_s_fw<-sum(salmon_fw_df$gall_peter_farm_marine_salmon_aquaculture_meat_water_per_cell,na.rm = T)
all_c_nutri<-sum(chicken_nutri_df$gall_peter_farm_land_chickens_industrial_meat_nutrient_per_cell,na.rm = T)
all_s_nutri<-sum(salmon_nutri_df$gall_peter_farm_marine_salmon_aquaculture_meat_nutrient_per_cell,na.rm = T)
all_c_ghg<-sum(chicken_ghg_df$gall_peter_farm_land_chickens_industrial_meat_ghg_per_cell,na.rm = T)
all_s_ghg<-sum(salmon_ghg_df$gall_peter_farm_marine_salmon_aquaculture_meat_ghg_per_cell,na.rm = T)
```
...
# What's the difference between this and the "all" df above? Values differ
...
chick_land_dist<-animal_stressor_location %>% 
...
chick_land_dist/all_chick_dist

#Salmon percent ocean including open ocean

salmon_ocean_dist<-animal_stressor_location %>% 
...
(salmon_ocean_dist + open_ocean_salmon)/all_salmon_dist

# GHG location
animal_stressor_location %>% filter(animal_system == "chickens", stressor == "ghg") #99.9% on land
animal_stressor_location %>% filter(animal_system == "salmon", stressor == "ghg") # 76% in marine

# Freshwater location
animal_stressor_location %>% filter(animal_system == "chickens", stressor == "water") #100% on land
animal_stressor_location %>% filter(animal_system == "salmon", stressor == "water") #100% on land

# Nutrient location
animal_stressor_location %>% filter(animal_system == "chickens", stressor == "nutrient") #100% on land
...
chicken_dist<- animal_stressor%>% filter(animal_system == "chickens", stressor == "disturbance") %>% ungroup() %>% dplyr::select(total_dist)
salmon_dist<- animal_stressor%>% filter(animal_system == "salmon", stressor == "disturbance") %>% ungroup() %>% dplyr::select(total_dist)
chicken_ghg<-animal_stressor%>% filter(animal_system == "chickens", stressor == "ghg") %>% ungroup() %>% dplyr::select(total_dist)
salmon_ghg<-animal_stressor%>% filter(animal_system == "salmon", stressor == "ghg") %>% ungroup %>% dplyr::select(total_dist)
chicken_water<-animal_stressor%>% filter(animal_system == "chickens", stressor == "water") %>% ungroup() %>% dplyr::select(total_dist)
salmon_water<-animal_stressor%>% filter(animal_system == "salmon", stressor == "water") %>% ungroup() %>% dplyr::select(total_dist)
chicken_nutrients<-animal_stressor%>% filter(animal_system == "chickens", stressor == "nutrient") %>% ungroup() %>% dplyr::select(total_dist)
...
quant_0 <- quantile(chicken_quant, 0) #0.00000000000000000008072476   
quant_45 <- quantile(chicken_quant, 0.45) #0.00002226699 
quant_90 <- quantile(chicken_quant, 0.90) #0.2164312
...
length(which(chicken_df$chickens_cumulative_stress>quant_90))/nrow(chicken_df)
```
...
quant_0 <- quantile(salmon_quant, 0) #0.000000000000007897534 
quant_45 <- quantile(salmon_quant, 0.45) #0.00002296986  
quant_90 <- quantile(salmon_quant, 0.90) #0.008789727
...
```{r}
test<-chicken_df %>% 
  filter(!is.na(chickens_cumulative_stress) == T,
         chickens_cumulative_stress>0) %>% 
  arrange(desc(chickens_cumulative_stress))

all_chick<-sum(test$chickens_cumulative_stress)

test<-test %>% 
  mutate(cum = cumsum(chickens_cumulative_stress))
...
Not sure which of the below to use?
```{r}

broilers <- list.files("/home/shares/food-systems/Food_footprint/chicken_salmon/rescaled_cumulative_pressure/system_stressor", pattern="chicken", full=TRUE)
...
nutri_rescale<-as.numeric(rescaling_values[2,2])

broiler_prop<-broiler_df %>% 
  as.data.frame() %>% 
  rename(disturbance = chickens_disturbance_rescaled,
         ghg = chickens_ghg_rescaled,
         nutrient = chickens_nutrient_rescaled,
         water = chickens_water_rescaled) %>% 
...
```{r}
broilers2 <- list.files("/home/shares/food-systems/Food_footprint/chicken_salmon/raw_pressure_summary/system_stressor", pattern="chicken", full=TRUE)

broiler_stack2<-stack(broilers2)
broiler_df2<-rasterToPoints(broiler_stack2)

broiler_prop2<-broiler_df2 %>% 
  as.data.frame() %>% 
...
max(broiler_prop2$total_chi)
sum(broiler_prop2$total_chi)
```
...
salmon_prop<-salmon_df %>% 
  as.data.frame() %>% 
  rename(disturbance = salmon_disturbance_rescaled,
         ghg = salmon_ghg_rescaled,
         nutrient = salmon_nutrient_rescaled,
         water = salmon_water_rescaled) %>% 
   mutate(disturbance_cell_prop = disturbance/dist_rescale,
         water_cell_prop = water/water_rescale,
         ghg_cell_prop = ghg/ghg_rescale,
         nutrient_cell_prop = nutrient/nutri_rescale) %>% 
...
```{r}
salmon2 <- list.files("/home/shares/food-systems/Food_footprint/chicken_salmon/raw_pressure_summary/system_stressor", pattern="salmon", full=TRUE)
salmon_stack2 <- stack(salmon2)
salmon_df2<-rasterToPoints(salmon_stack2)

salmon_prop2<-salmon_df2 %>% 
  as.data.frame() %>% 
...
Will need to add these to repo?
```{r}
...
animal_stressor_feed_location %>% 
  filter(animal_system == "chickens", stressor == "disturbance") #99.7% feed

animal_stressor_feed_location %>% 
  filter(animal_system == "salmon", stressor == "disturbance") # 99.9% feed
