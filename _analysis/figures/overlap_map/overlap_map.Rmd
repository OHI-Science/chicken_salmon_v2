...
chicken_df_no0<-chicken_df %>% 
  filter(!broilers_cumulative_stress<=0)
...
sub_disturb_quant<- disturb_quant %>% 
  filter(broilers_cumulative_stress<quant_45)
...
length(which(recl_chicken_df$broilers_cumulative_stress == 3))
```
...
combo_assign <- combo %>% 
  ...
  rename(chicken = broilers_cumulative_stress) %>% 
  rowwise() %>% 
...
par(mar=c(1,1,1,1))
plot <- raster::plot(rast_combo_assign, breaks=cuts, col=col_cont, add = T)
```
...
legend_graph <- ggplot(data = legend, aes(x = salmon, y = chicken)) +
  ...
  theme_minimal() +
  scale_fill_manual("value", values=col_discrete)+
...
legend_graph
ggsave("_analysis/overlap_map/outputs/overlap_legend.jpg", width=5, height=3, dpi=300)
...
grDevices::png(here("_analysis/figures/overlap_map/outputs/overlap_map.png"), res=500, width=6, height=3, units = "in")
...
bar_graph <- ggplot(cell_count, aes(x = category, y = number_cells/1000000, fill= color_assign))+
  ...
  scale_fill_manual("color_assign", values=col_discrete)
 
bar_graph

ggsave("_analysis/figures/overlap_map/outputs/bar_graph_with_zero.jpg",height = 5, width = 6,  dpi=300) 
...
circle_legend_plot <- ggplot(circle_legend, aes(x= salmon, y=chicken)) +
  ...
  scale_color_manual("color_assign", values=col_discrete) +
...
ggsave("_analysis/figures/overlap_map/outputs/overlap_circle_legend.jpg", width=6, height=4, dpi=300) 
```
