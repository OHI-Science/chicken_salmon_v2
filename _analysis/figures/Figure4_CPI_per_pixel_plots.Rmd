...
iso3_broiler_cpi<-iso3_broiler_cpi %>% 
  dplyr::select(-variance_chi, )
...
```{r}
chick_cpi_km2_plot+coord_trans(y = "log10") 
```
```{r}
ggsave(here("_analysis/figures/Chicken_cpi_km2_log10.png"), dpi = 300, height = 4, width = 6)
```
# Salmon
...
```{r}
salmon_cpi_km2_plot+coord_trans(y = "log10")
```


```{r}
ggsave(here("_analysis/figures/Salmon_cpi_km2_log10.png"),dpi = 300, height = 4, width = 6)
```