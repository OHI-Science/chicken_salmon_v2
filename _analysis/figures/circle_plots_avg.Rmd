...
pressures <- read_csv(here("_analysis/data/zonal_extract/sum_pressures_country.csv"))  %>%
  ...
  left_join(eez_rgns) %>%
  rowwise() %>%
  mutate(avg_cum_stress_km2_1e_9= ifelse(location=="marine", 
                          country_farm_feed_chi/eez_area_km2*1000000000,
                          country_farm_feed_chi/land_area_km2*1000000000)) %>%
  ungroup() %>%
   mutate(prop_of_global = ifelse(is.na(avg_cum_stress_km2_1e_9), 0, avg_cum_stress_km2_1e_9)) %>%
...
left_join(un_geopolitical, by="iso3c") %>%
  select(Country, iso3c, location, source, avg_cum_stress_km2_1e_9, georegion)
  

## get georegion ranks
chi <- pressures %>%
  group_by(georegion, Country, iso3c) %>%
  summarize(chi = sum(avg_cum_stress_km2_1e_9, na.rm=TRUE)) %>%
  arrange(-chi)
...
source_name <- data.frame(source = c("broiler-farm", "broiler-feed-crop", "salmon-feed-crop", 
                                         "salmon-farm", "broiler-feed-marinefish", "salmon-feed-marinefish"),
...
pressures_adj_circle <- pressures %>%
  ...
  mutate(chi = sum(avg_cum_stress_km2_1e_9, na.rm=TRUE)) %>%
  mutate(avg_cum_stress_km2_1e_9 = ifelse(location=="marine", avg_cum_stress_km2_1e_9*(-1), avg_cum_stress_km2_1e_9)) %>%
  ungroup() %>%
  arrange(chi) %>%
...
# ## add some blanks rows to match the master region list
  ...
  to_add$avg_cum_stress_km2_1e_9<-  0
...
pressures_adj_circle %>%
  group_by(source) %>%
  summarize(mean = mean(avg_cum_stress_km2_1e_9, na.rm=TRUE)) %>%
  arrange(mean)
...
p <- ggplot(data=pressures_adj_circle, aes(x=Country, y=avg_cum_stress_km2_1e_9, fill=source_name)) + 
...
p <- ggplot(data=pressures_adj_bar, aes(x=Country, y=avg_cum_stress_km2_1e_9, fill=source_name)) + 
  
  geom_bar(stat="identity") +
  ylab(bquote("cumulative pressures per" ~km^2~"(x 1e9)")) +
